function imOut = tonemap_clipping(imIn, type, value)
...
elseif (strcmpi(type, 'percentage')),
	clipLimit = maxv(imIn) * value;
elseif (strcmpi(type, 'absolute')),
	clipLimit = value;
elseif (strcmpi(type, 'absolute_bi') || strcmpi(type, 'percentile_bi') || strcmpi(type, 'percentage_bi')),
	if (numel(value) ~= 2),
	...
	elseif (strcmpi(type, 'percentage_bi')),
		clipLimit = [maxv(imIn) * value(1), maxv(imIn) * value(2)];
